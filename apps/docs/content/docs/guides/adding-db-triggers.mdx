...
```bash
pnpm db:trigger:new --name set_updated_at --table organizations
```
...
1. **Migration file** in `packages/infra/db/drizzle/migrations/` with a `CREATE TRIGGER` statement and a placeholder trigger function
2. **pgTAP test file** in `packages/infra/db/pgtap/` with a `fail()` placeholder that you must replace with real assertions
...
```sql
-- packages/infra/db/drizzle/migrations/XXXX_set_updated_at.sql
...
```sql
-- packages/infra/db/pgtap/set_updated_at.sql
BEGIN;
SELECT plan(2);

-- Verify the trigger exists
SELECT has_trigger('organizations', 'set_updated_at', 'Trigger set_updated_at exists on organizations');

-- Verify it fires on UPDATE
SELECT trigger_is('organizations', 'set_updated_at', 'set_updated_at',
...
Reference this trigger in `packages/infra/db/pgtap/*.sql`.
```
...
```bash
pnpm db:migrate && pnpm db:schemas:apply && pnpm test:db:pgtap && pnpm lint
```

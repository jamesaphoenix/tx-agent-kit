...
Schema contracts are the backbone of `<domain>`'s type safety across the full stack. Every API request and response is validated through `effect/Schema`, which also generates the OpenAPI specification that drives client codegen.

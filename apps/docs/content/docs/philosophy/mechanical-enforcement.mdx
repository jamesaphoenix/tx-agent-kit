...
The central principle of <domain> is: **if it is important, enforce it in code.** Conventions that exist only in documentation are conventions that will be violated.

## Three layers of enforcement

<domain> uses three complementary enforcement layers, each catching a different class of violations:
...
| No `console.*` in source modules | Unstructured logging; use `@<domain>/logging` instead |

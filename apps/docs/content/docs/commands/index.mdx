...
| Command | Description |
|---------|-------------|
| `pnpm dev` | Start all app dev servers in parallel (web, api, worker, mobile); when `DEV_CLOUDFLARE_TUNNEL_ENABLED=true`, starts a local Cloudflare tunnel and stops it on shutdown (lock-guarded across worktrees) |
...
| `pnpm dev:tunnel:status` | Show local dev tunnel lock ownership/status and owner metadata |
...
| `pnpm test:integration:live:k3s` | Opt-in live Mac `k3s` staging deployment integration (real infra, no mocks) |
| `pnpm test:integration:live:tunnel` | Opt-in live Cloudflare tunnel reconcile/check integration (real infra, no mocks) |
| `pnpm test:integration:live:deploy` | Run both live deploy integrations |
...
| `pnpm db:schemas:generate` | Regenerate desired-state schema SQL artifacts |
...
| `pnpm db:test:reset` | Reset mutable test DB state and reapply desired-state schemas |
...
| `pnpm deploy:k8s:mac:staging` | Deploy to Mac Studio `k3s` staging namespace/release from image artifact |
| `pnpm deploy:k8s:mac:prod` | Deploy to Mac Studio `k3s` prod namespace/release from image artifact |
| `pnpm deploy:k8s:gke` | Deploy to optional GKE target from the same image artifact |
| `pnpm deploy:k8s:verify:staging` | Dedicated Mac `k3s` staging deploy verification (separate from standard integration suites) |
| `pnpm deploy:k8s:status` | Show Helm + Kubernetes status for `mac-staging`, `mac-prod`, or `gke` |
| `pnpm deploy:k8s:rollback` | Roll back Helm release for `mac-staging`, `mac-prod`, or `gke` |
| `pnpm deploy:tunnel:reconcile` | Render/validate Cloudflare tunnel routing config for Mac-hosted API exposure |
| `pnpm deploy:tunnel:check` | Verify Cloudflare tunnel `/health` checks for dev/staging/prod hostnames |
...
| `pnpm test:gcp:e2e:gke` | Run OTEL GCP E2E plus optional GKE deploy/setup/teardown validation |
...
| `pnpm playwright:auth:bootstrap` | Bootstrap authenticated Playwright storage state via real auth API |
...
## Parallel integration runs across worktrees

Integration tests run in parallel across git worktrees without file locking. `scripts/worktree/setup.sh` allocates a deterministic `WORKTREE_PORT_OFFSET` per worktree, and `scripts/test/vitest-global-setup.ts` binds the shared integration API server to `4100 + offset`. Each worktree also gets its own Postgres schema, so database state is already isolated.

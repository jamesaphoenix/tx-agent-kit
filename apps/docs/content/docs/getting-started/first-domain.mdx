...
If you added database columns, run the migration pipeline:

```bash
pnpm db:migrate       # Apply Drizzle migrations from packages/infra/db/drizzle/migrations/
pnpm db:schemas:apply # Reconcile desired-state schema SQL + enum sync
```

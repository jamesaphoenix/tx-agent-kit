...
description: Detailed breakdown of every app and package in the <domain> monorepo
---
...
The API server is built with Effect HttpApi, providing typed REST endpoints for all domain operations. It handles authentication (sign-up, sign-in, JWT token management), CRUD operations for organizations and invitations, OpenAPI spec generation, and request validation using `effect/Schema` contracts.

...
Integration tests are safe to run in parallel across git worktrees:

- **Per-worktree Postgres schema.** `scripts/worktree/setup.sh` creates a
  dedicated schema named after the worktree and writes it into the
  worktree's `DATABASE_URL`.
- **Per-worktree shared API port.** `scripts/test/vitest-global-setup.ts`
  binds the shared integration API server to `4100 + WORKTREE_PORT_OFFSET`.
- **Pool-slot claim files inside each worktree.** Web integration workers
  claim slots under `.vitest/web-integration/slot-claims`, which is local
  to the worktree's filesystem.

| Lock File | Purpose |
|-----------|---------|
| `/tmp/tx-agent-kit-db-reset.lock` | Guards database reset within a single checkout |

No global integration suite lock is used — parallelism across worktrees
comes from port and schema isolation, not from serialization.

## Database Reset

The testkit provides idempotent database reset that truncates test data tables while preserving the schema and migrations, scoped to each worktree's isolated schema.

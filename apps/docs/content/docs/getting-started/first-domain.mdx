...
The fastest way to add a new feature to <domain> is the scaffold CLI. It generates a complete DDD slice (domain types, ports, application logic, adapters, routes, and tests) following every convention and passing all lint checks out of the box.
...
Would create:
  ...
  packages/infra/db/src/effect-schemas/invoice.ts
  packages/infra/db/src/factories/invoice.factory.ts
...
| Database | `packages/infra/db/` | Effect schema aligned with Drizzle table, factory for test data, repository implementing the port |
...
2. Add the database columns to the Drizzle schema in `packages/infra/db/src/schema.ts`
3. Update the effect schema in `packages/infra/db/src/effect-schemas/invoice.ts` to match
4. Update the factory in `packages/infra/db/src/factories/invoice.factory.ts`

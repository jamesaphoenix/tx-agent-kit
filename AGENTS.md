...
- For normal `apps/web` runtime API access, pages/components/hooks must use generated Orval hooks/functions in `apps/web/lib/api/generated/*`. Reserve `apps/web/lib/client-api.ts` for auth/session bootstrap, sign-in/sign-out side effects, and imperative test/setup helpers. Any runtime exception must be documented inline.

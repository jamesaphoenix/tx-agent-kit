...
<domain> supports parallel development through git worktrees. Each worktree gets deterministic port assignments and an isolated database schema, while sharing the same Docker infrastructure containers.
...
| `MOBILE_PORT` | `8223` |
| `WORKER_INSPECT_PORT` | `9371` |
| `TEMPORAL_TASK_QUEUE` | `<domain>-feature-auth` |
...
`MOBILE_PORT` is the Expo/Metro endpoint port used by `pnpm dev:mobile`. Because it is derived from `WORKTREE_PORT_OFFSET`, mobile endpoint URLs (for example, `http://localhost:<MOBILE_PORT>`) are also worktree-specific.

## Shared infrastructure

All worktrees share the same Docker containers. The `docker-compose.yml` project name is pinned to `<domain>`, so `pnpm infra:ensure` from any worktree connects to the same running services.

### Isolated per worktree

| Resource | Isolation mechanism |
|----------|-------------------|
| Database schema | Each worktree gets its own PostgreSQL schema within `<domain_identifier>` |
| Temporal task queue | Each worktree uses a unique queue name (`<domain>-<name>`), preventing workers from competing for the same workflows |

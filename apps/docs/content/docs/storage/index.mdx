...
|-------------|--------|-----------|
| Development | `tx-agent-kit-dev` | Local dev, CI |
| Staging | `tx-agent-kit-staging` | Pre-production testing |
| Production | `tx-agent-kit-prod` | Live traffic |
...
- **1Password for credentials.** R2 access keys are stored in the `tx-agent-kit-services` vault and injected via `op://` references.

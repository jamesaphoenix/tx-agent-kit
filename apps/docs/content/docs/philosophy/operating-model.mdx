...
`<domain>` defines a clear operating model for how humans and agents collaborate. This model is not aspirational. It is the actual workflow the repository is designed to support.
...
**Human:** "Add a `billing` domain with an `invoice` entity. Invoices have an amount, currency, status (draft/sent/paid), and belong to an organization."

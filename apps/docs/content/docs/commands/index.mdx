...
Integration tests are guarded by a file lock at `/tmp/<domain>-integration.lock`. This prevents overlapping integration test runs from clobbering each other's database state. If a previous run was interrupted, the lock file is automatically cleaned up by the next run.

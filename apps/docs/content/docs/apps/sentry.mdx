...
For local error visibility without a Sentry account, enable [Sentry Spotlight](/docs/observability/spotlight). Set `SENTRY_SPOTLIGHT=true` in your `.env` and run the Spotlight sidecar — all three layers forward errors and traces to `http://localhost:8969`.

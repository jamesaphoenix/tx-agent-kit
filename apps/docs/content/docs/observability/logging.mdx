---
title: Logging
description: Structured JSON logging via @<domain>/logging with OTEL log record emission
---

All logging in <domain> goes through the `@<domain>/logging` package. Direct `console.*` calls are banned by ESLint across all source modules.
...
```typescript
import { logError } from '@<domain>/logging'
...
```typescript
import { createPerfLogger } from '@<domain>/logging'
...
```typescript
import { logStateChange } from '@<domain>/logging'
...
```typescript
import { logProgress } from '@<domain>/logging'
...
```typescript
import { defaultLogger } from '@<domain>/logging'

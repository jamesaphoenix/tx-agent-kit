...
description: How domain-driven design is implemented in <domain>
---

`<domain>` implements domain-driven design (DDD) with a specific construction pattern that separates concerns into well-defined layers. This pattern is enforced mechanically, not just documented.
...
import type { InvoiceRepositoryPort } from "@<domain>/core/domains/billing/ports/invoice-repository.port.js"
...
| No `console.*` calls | Use `@<domain>/logging` |

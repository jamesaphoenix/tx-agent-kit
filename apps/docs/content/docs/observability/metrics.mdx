...
`<domain>` collects application metrics using the OpenTelemetry SDK. Metrics are exported via OTLP to the collector, which routes them to Prometheus locally or GCP Cloud Monitoring in production.

## How metrics work

The `@<domain>/observability` package initializes a `PeriodicExportingMetricReader` that pushes metrics every 5 seconds:
...
```typescript
import { getOrCreateNodeServiceMetrics } from '@<domain>/observability'

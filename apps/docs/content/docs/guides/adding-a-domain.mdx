...
The `events.ts` file at the domain root is structurally significant — it is the ONLY file other domains may import from this one. Everything else is private to the slice. See [DDD Pattern → Cross-domain boundaries](/docs/architecture/ddd-pattern#cross-domain-boundaries) for the rules and enforcement.
...
| File | Contents |
|------|----------|
| `packages/infra/db/src/effect-schemas/invoices.ts` | Effect schema with `InvoicesRowSchema` and `InvoicesRowShape` |
| `packages/infra/db/src/factories/invoices.factory.ts` | Test factory with `createInvoicesFactory` |

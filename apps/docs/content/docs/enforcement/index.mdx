...
| Code quality | Unused import removal, curly braces, object shorthand, arrow callbacks |
| Modern JS (unicorn) | `node:` protocol, `.replaceAll()`, `.slice()`, `.flatMap()`, `structuredClone` |
| Test governance | No `.only()` in test files, no constant binary expressions |

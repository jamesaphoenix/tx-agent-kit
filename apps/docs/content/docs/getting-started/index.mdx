...
description: Prerequisites and quick overview for setting up <domain>
---

This section walks you through setting up <domain> for local development, from prerequisites to running your first domain scaffold.

## Opinionated stack

<domain> is deliberately opinionated. It picks one tool per concern and enforces that choice mechanically through linters, structural checks, and shell invariants. This means the prerequisites are non-negotiable: you need the exact toolchain below, not alternatives.
...
<domain> is a pnpm monorepo with four applications and several shared packages:

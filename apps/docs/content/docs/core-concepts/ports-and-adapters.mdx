...
The ports and adapters pattern (also known as hexagonal architecture) is central to <domain>'s design. Ports define abstract capability contracts. Adapters implement those contracts with concrete infrastructure.
...
Ports in <domain> follow strict conventions enforced by structural invariant checks.
...
Adapters implement port contracts using specific technologies. In <domain>, the primary adapters are Drizzle-based repository implementations in `packages/infra/db/src/repositories/`:
...
import type { OrganizationRepositoryPort } from "@<domain>/core/domains/organization/ports/organization-repository.port.js"
...
In many hexagonal architecture implementations, adapters live next to the domain. <domain> takes a different approach: **persistence adapters live in `packages/infra/db/src/repositories/`**.
...
import { OrganizationRepository } from "@<domain>/core/domains/organization/ports/organization-repository.port.js"
import { makeOrganizationRepository } from "@<domain>/db/repositories/organizations.js"

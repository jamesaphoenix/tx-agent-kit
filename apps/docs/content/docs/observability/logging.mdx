...
logger.error('Request failed', { path: '/v1/organizations' }, error)
```
...
dbLogger.info('Query executed', { table: 'organizations', duration: 42 })
```
...
const perf = createPerfLogger(logger, 'database.query', { table: 'organizations' })
...
The logger respects the `LOG_LEVEL` environment variable. Logs below the configured level are silently dropped (no stdout, no OTEL emit).

| `NODE_ENV` | Default `LOG_LEVEL` |
|---|---|
| `development` / `test` | `debug` |
| `staging` / `production` | `warn` |

Valid values are `debug`, `info`, `warn`, and `error`. Set `LOG_LEVEL` explicitly in `.env` to override the default:

```bash
# In root .env — override to info in local dev
LOG_LEVEL=info
```

Staging and production deploy templates (`deploy/env/staging.env.template`, `deploy/env/prod.env.template`) default to `warn`.

Child loggers inherit the parent's log level automatically.

...
The logger filters output based on the `LOG_LEVEL` environment variable. Logs below the threshold are dropped entirely (no stdout, no OTEL emit).
...
Override by setting `LOG_LEVEL` in your root `.env`:

```bash
LOG_LEVEL=info
```

Valid values: `debug`, `info`, `warn`, `error`. Invalid values fall back to the NODE_ENV-based default. Child loggers inherit the parent's level.

## Environment

| Variable | Purpose | Default |
|---|---|---|
| `SERVICE_NAME` | Logger service name | `tx-agent-kit` |
| `LOG_LEVEL` | Minimum log severity | `debug` (dev) / `warn` (staging/prod) |

Both are read from `packages/infra/logging/src/env.ts`.

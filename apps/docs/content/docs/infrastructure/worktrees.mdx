...
The setup script generates two helper scripts inside the worktree. `run-migrations.sh` runs `pnpm db:migrate` followed by `pnpm db:schemas:apply` inside the worktree context. `reset-worktree-schema.sh` drops and recreates the worktree's PostgreSQL schema.

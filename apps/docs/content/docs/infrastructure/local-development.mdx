...
| `LANGFUSE_PUBLIC_KEY` | local placeholder key |
...
This starts all Docker Compose services under the `infra` profile and waits for health checks to pass. Services include PostgreSQL, Redis, the OTEL collector, Jaeger, Prometheus, Grafana, Loki, Spotlight, and Langfuse.

Langfuse local bootstrap runs as part of `pnpm infra:ensure`. It ensures the local `tx-agent-kit Local` organization, project, and `dev@tx-agent-kit.local` owner account exist and resets the local password to `tx-agent-kit-local-langfuse`.
...
Drizzle ORM migrations run against the database specified in `DATABASE_URL`. Reapplied desired-state SQL then runs from `packages/infra/db/schemas/`.
...
Run this after `pnpm dev` is up. It opens local app/dev dashboards (web, mobile Metro status, API Swagger, Temporal UI, Grafana, Prometheus, Jaeger, Langfuse, Spotlight) in Brave Browser with a Google Chrome fallback.

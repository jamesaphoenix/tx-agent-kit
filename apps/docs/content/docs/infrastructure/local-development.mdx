...
The local development workflow follows a strict sequence: configure environment, start infrastructure, run migrations, start applications.
...
This starts all Docker Compose services under the `infra` profile and waits for health checks to pass. Services include PostgreSQL, Temporal, Redis, the OTEL collector, Jaeger, Prometheus, Grafana, and Loki.

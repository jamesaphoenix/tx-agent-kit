---
title: Core Concepts
description: Overview of the core technical concepts used in <domain>
---

<domain> is built on a small set of technical concepts that work together to produce a maintainable, testable, and agent-friendly codebase. This section explains each concept in depth.
...
|-----------|------|------------|
| **Effect** | Dependency injection, typed errors, service composition | `packages/core`, `packages/infra/db`, `apps/api`, `apps/worker` |
| **effect/Schema** | Validation, serialization, API contracts | `packages/contracts`, `packages/infra/db/src/effect-schemas` |
| **Drizzle ORM** | Database schema, migrations, queries | `packages/infra/db` only |
...
5. The port is implemented by an **adapter** (Drizzle repository in `packages/infra/db`).

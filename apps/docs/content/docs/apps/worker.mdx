...
export const activities = {
  processOperation: async (operationId: string) => {
    // Database queries, API calls, etc. are allowed here
  }
}
```

The `processOperation` activity demonstrates idempotent processing: calling it twice for the same operation returns `{ alreadyProcessed: true }` on the second call.
...
The worker Temporal integration suite at `apps/worker/src/activities.integration.test.ts` validates that first-time operation processing returns `{ alreadyProcessed: false }`, duplicate processing returns `{ alreadyProcessed: true }`, and test fixtures are seeded per test (no shared table truncation).

...
- If the user ran `/plan` or was in plan mode, the plan text is in the conversation. Write it to `~/.codex/plans/<name>.md` (relative to repo root).
- If a plan file already exists at `~/.codex/plans/<name>.md`, read it instead of overwriting.
- If no plan but the user gave detailed requirements, generate a plan yourself: research the codebase, think through requirements/edge cases/constraints, write the plan to `~/.codex/plans/<name>.md`.
- If no plan and the request is vague, ask the user to run `/plan` first or provide more detail.

The plan is saved as a standalone file at `~/.codex/plans/<name>.md` (relative to repo root). The doc's frontmatter gets `plan: ~/.codex/plans/<name>.md` and the `# Plan` section contains a reference link plus a brief summary, not the full verbatim content.
...
ears_requirements:
  - id: REQ-<SCOPE>-400
    kind: unwanted
    statement: if <condition>, then the system shall <action>
    priority: must
    rationale: <why>
```

...
`<domain>` is built with OpenTelemetry-native observability. All three signal types (traces, metrics, and logs) are exported via OTLP to a centralized collector that routes them to the appropriate backend.
...
| Page | Description |
|------|-------------|
| [Logging](/docs/observability/logging) | Structured logging with `@<domain>/logging` |

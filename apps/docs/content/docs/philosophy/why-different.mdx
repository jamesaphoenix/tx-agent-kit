---
title: Why <domain> is Different
description: How this kit differs from typical boilerplates by encoding taste in mechanical checks
---

Most starter kits and boilerplates give you a file structure and some example code. <domain> gives you an **architecture enforcement system** that happens to also include a file structure and example code.
...
<domain> takes every architectural decision and asks: **can we enforce this mechanically?**
...
| No `console.*` in source | Lint ban; use `@<domain>/logging` instead |
...
Beyond enforcement, <domain> provides a scaffold CLI that generates correct-by-construction code:

```bash
pnpm scaffold:crud --domain billing --entity invoice --dry-run
```

This generates the full DDD slice: domain, ports, application logic, adapters, routes, and tests. It follows every convention automatically. The generated code passes all lint checks and type checks out of the box.

...
`<domain>` enforces a strict inward-only dependency direction. Inner layers never depend on outer layers. This is not a guideline. It is mechanically enforced by ESLint boundary rules and structural invariant checks.
...
| Only `packages/infra/db` may import `drizzle-orm` | ESLint + structural check |
| Source modules cannot use `console.*` | ESLint (use `@<domain>/logging`) |
...
**Replaceability.** Switching from PostgreSQL to another database means replacing repository implementations in `packages/infra/db`. The domain layer, ports, and application logic do not change.
...
Consider adding a validation rule that invoices cannot exceed an organization's credit limit:

1. **Domain layer**: Add a pure function `isWithinCreditLimit(invoice, organization)` to `domain/`
2. **Port layer**: No changes needed (organization data comes through existing ports)

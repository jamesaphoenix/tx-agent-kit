...
`<domain>` uses OpenTelemetry for distributed tracing. The `@<domain>/observability` package bootstraps the trace provider, and all spans are exported via OTLP to the collector.
...
```typescript
import { startTelemetry } from '@<domain>/observability'

await startTelemetry('api')
```

This registers a `NodeSDK` instance with an `OTLPTraceExporter` pointing at `OTEL_EXPORTER_OTLP_ENDPOINT/v1/traces`, a resource with `service.name` and `deployment.environment.name` attributes, and automatic span context propagation.
...
```typescript
import { emitNodeTelemetrySmoke } from '@<domain>/observability'
...
```typescript
import { stopTelemetry } from '@<domain>/observability'

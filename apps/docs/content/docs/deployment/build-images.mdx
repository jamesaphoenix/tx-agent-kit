...
Default push target is Google Artifact Registry:

```bash
${ARTIFACT_REGISTRY_REGION}-docker.pkg.dev/${GCP_PROJECT_ID}/${ARTIFACT_REGISTRY_REPOSITORY}/api
${ARTIFACT_REGISTRY_REGION}-docker.pkg.dev/${GCP_PROJECT_ID}/${ARTIFACT_REGISTRY_REPOSITORY}/worker
```
...
| Image | Source | Base |
|-------|--------|------|
| `<domain>-api` | `apps/api` | Node.js 22 Alpine |
| `<domain>-worker` | `apps/worker` | Node.js 22 Alpine |
...
```bash
API_IMAGE=registry.example.com/<domain>-api@sha256:abc123...
WORKER_IMAGE=registry.example.com/<domain>-worker@sha256:def456...
```
...
# Deploy Mac k3s staging with the same artifact
pnpm deploy:k8s:mac:staging deploy/artifacts/images-a1b2c3d.env
```

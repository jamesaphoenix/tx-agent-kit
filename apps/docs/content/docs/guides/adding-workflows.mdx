...
export const activities = {
  processOperation: async (operationId: string) => {
    ...
    return { alreadyProcessed: false, operationId }
  },
...
const { processOperation, sendNotification } = proxyActivities<typeof activities>({
  startToCloseTimeout: '30s'
})

export async function operationProcessingWorkflow(operationId: string): Promise<void> {
  const result = await processOperation(operationId)

  if (!result.alreadyProcessed) {
    await sendNotification(result.userId, `Operation ${operationId} processed`)
  }
}
```
...
const first = await activities.processOperation(operationId)
expect(first.alreadyProcessed).toBe(false)

// Second call detects the duplicate
const second = await activities.processOperation(operationId)

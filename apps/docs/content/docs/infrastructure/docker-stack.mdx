---
title: Docker Stack
description: Docker Compose services for PostgreSQL, Redis, and the observability stack
---

All shared infrastructure runs through `docker-compose.yml` under the `infra` profile. The Docker Compose project name is pinned to `<domain>` to allow sharing across multiple git worktrees.
...
The Docker Compose project is pinned to `<domain>` regardless of which directory the command runs from. This ensures all worktrees share the same infrastructure containers, avoiding duplicate service instances.

...
import { type SubscriptionStatus, subscriptionStatuses } from '@<domain>/contracts'

// Used in API route validation
import { createOrganizationRequestSchema } from '@<domain>/contracts'
```
...
```typescript
import { domainEventTypes, domainEventAggregateTypes, domainEventStatuses } from '@<domain>/contracts'
```
